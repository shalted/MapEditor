Shader "BackOutline"
{
    Properties
    {
	    _MainTex("主贴图 A透明通道", 2D) = "white" {}
    	_BaseColor("主颜色 A通道透明拉条", Color) = (1, 1, 1, 1)
        _ShadowMainColor("阴影颜色", Color) = (0.6,0.6,0.6,1)
        _ShadowOffest("阴影偏移", Range( -1 , 1)) = 0.5
        _ShadowSmooth("阴影软硬", Range( 0 , 0.499)) = 0.499
    	...
    	[CustomToggle(_RimSetting._USERIM)]_AddRamp("默认替换模式，更换为叠加模式",float) = 0 
        [CustomObject(_RimSetting._USERIM)][HDR]_FresnelColor("内层边缘光颜色", Color) = (0,0,0,0)
        [CustomObject(_RimSetting._USERIM)]_RimLightEffect("内层边缘光光照影响", Range( 0 , 1)) = 0
    ...
    SubShader
    {
		...
		HLSLINCLUDE
			...
			half4 _BaseColor,_ShadowMainColor,//基础设置
			...
			_ProceduralColor;//程序控制
        
			float4 _Noise_ST;

			float _OutlineWidth, _ScaleDepth;//描边设置
        
			half _NormalIntensity,_ShadowSmooth,_ShadowOffest, //基础设置
			...
			_Scale, _OutlineSpeed, _Threshold; //描边设置
            CBUFFER_END
    	...
    	Pass
        {
			...
			half4 frag ( v2f i  ) : SV_Target
			{
				...
				float NdotL = dot( worldNormal , _MainLightPosition.xyz );
				float NdotLOffest = ( _ShadowOffest + (NdotL * 0.5 + 0.5) );
				NdotLOffest = smoothstep( _ShadowSmooth , ( 1.0 - _ShadowSmooth ) , NdotLOffest);
				float3 finalColor = lerp( MainTex * _ShadowMainColor , MainTex * _BaseColor, NdotLOffest);
				
				float3 worldPos = i.worldPos.xyz;
				float3 worldViewDir = normalize( _WorldSpaceCameraPos.xyz - worldPos );
				float NdotV = dot( worldNormal, worldViewDir );
				//================边缘光计算==================
            	half rimHalfLambert = dot(-_RimLightDir.xyz, worldNormal) * 0.5 + 0.5;
            	...
            	half3 FresnelColor = _FresnelColor.xyz * lerp(half3(1, 1, 1), MainTex.rgb, _RimColorBlend);
				finalColor.xyz = lerp( finalColor.xyz , lerp( FresnelColor, finalColor.xyz + FresnelColor , _AddRamp), ( RimTerm * LightAffect * _useRim));


				//================matcap计算==================
				float2 uv_MatcapMask = i.uv.xy * _MatcapMask_ST.xy + _MatcapMask_ST.zw;
				half4 matcapColor = SAMPLE_TEXTURE2D( _MatcapTex,sampler_MatcapTex, (mul( UNITY_MATRIX_V, half4( i.worldNormal.xyz , 0.0 ) ).xyz*0.5 + 0.5).xy );
				...
				half4 col;
				col.xyz = finalColor.rgb * _ProceduralColor.rgb;
				col.w = MainTex.a * _ProceduralColor.a * _BaseColor.a;
				return col;
			}
            ENDHLSL
        }
	    ...
	    Pass
    	{
			...
			half4 frag ( v2f i  ) : SV_Target
			{
				...
				#if _DyeColor
				    float index = 0.25;
				    float idx =clamp(round(i.color.r * 10),0,9);
				    float idy;
				    if ( idx > 6)
				    {
				        idy = 3;
