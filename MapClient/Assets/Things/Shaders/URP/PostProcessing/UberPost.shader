Shader "Hidden/Universal Render Pipeline/UberPost"
{
    HLSLINCLUDE
        ...
        #pragma multi_compile_fragment _ _MIXBLOOM_ON
        ...
        half4 FragUberPost(Varyings input) : SV_Target
        {
            ...
            #elif defined(_MIXBLOOM_ON)
            {
                  half4 mixbloom = SAMPLE_TEXTURE2D_X(_MixBloomTexture, sampler_LinearClamp, uvDistorted);
                  #if UNITY_COLORSPACE_GAMMA
                       mixbloom.xyz *= mixbloom.xyz; // γ to linear
                  #endif
                   
                  UNITY_BRANCH
                  if (BloomRGBM > 0)
                  {
                      mixbloom.xyz = DecodeRGBM(mixbloom);
                  }
                  mixbloom.xyz *= _BloomIntensity;
                  color += mixbloom.xyz;// * BloomTint;
