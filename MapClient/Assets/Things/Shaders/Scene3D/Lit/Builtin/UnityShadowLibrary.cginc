...
fixed UnitySampleBakedOcclusion (float2 lightmapUV, float3 worldPos, out half4 shadowmask)
{
    shadowmask = 0;
    #if defined (SHADOWS_SHADOWMASK)
        #if defined(LIGHTMAP_ON)
            fixed4 rawOcclusionMask = UNITY_SAMPLE_TEX2D(unity_ShadowMask, lightmapUV.xy);
            shadowmask = rawOcclusionMask;
        #else
            ...
            half4 shadowmaskFade = SampleShadowmaskFade(shadowmask, lightmapUV.xy);
            rawOcclusionMask = lerp(rawOcclusionMask, shadowmaskFade, _GlobalSceneFadeWeight);
        #endif
...
{
    half4 shadowmask;
    return UnitySampleBakedOcclusion(lightmapUV, worldPos, shadowmask);
}
// ------------------------------------------------------------------
