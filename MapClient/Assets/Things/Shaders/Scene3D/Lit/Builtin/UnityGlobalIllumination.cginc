...
inline UnityGI UnityGI_Base(UnityGIInput data, half occlusion, half3 normalWorld)
{
    ...
    o_gi.light.color *= data.atten;
        ...
        // Baked lightmaps
        half4 bakedColorTex = SampleLightmap(shadowmask, data.lightmapUV.xy);
...
inline half3 UnityGI_IndirectSpecular(UnityGIInput data, half occlusion, Unity_GlossyEnvironmentData glossIn)
{
    ...
    specular = Unity_GlossyEnvironment (UNITY_PASS_TEXCUBE(_AmbientSpecCube), half4(1, 1, 0, 0), glossIn);
#ifdef USE_GLOBAL_SCENE_FADE
    half3 fadeSpecular = Unity_GlossyEnvironment (UNITY_PASS_TEXCUBE(_FadeAmbientSpecCube), half4(1, 1, 0, 0), glossIn);
    specular = lerp(specular, fadeSpecular, _GlobalSceneFadeWeight);
#endif
    specular *= ACCESS_COMBINE_DC_PROP(_AmbientSpecularScale);
