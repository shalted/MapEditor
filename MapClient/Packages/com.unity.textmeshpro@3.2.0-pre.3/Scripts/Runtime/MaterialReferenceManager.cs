...
namespace TMPro
{
    ...
    public class MaterialReferenceManager
    {
        ...
        private Dictionary<TMP_SpriteAsset, int> m_SpriteAssetCount = new Dictionary<TMP_SpriteAsset, int>();
        /// 
        public static event Action<string> OnReleaseAsset;
        ...
        public static MaterialReferenceManager instance
        {
            get
            {
                if (MaterialReferenceManager.s_Instance == null)
                {
                    ...
                    TMP_SubMeshUI.OnEnableRequest += (x) =>
                    {
                        if (x == null || x.name == "NullDontDelete") return;
                        if (!s_Instance.m_SpriteAssetCount.ContainsKey(x))
                        {
                            s_Instance.m_SpriteAssetCount.Add(x, 0);
                        }
                        s_Instance.m_SpriteAssetCount[x]++;
                        //Debug.Log($"表情资源启动{x.name}+{s_Instance.m_SpriteAssetCount[x]}");
                    };
                    TMP_SubMeshUI.OnDisableRequest += (x) =>
                    {
                        if (x == null || x.name == "NullDontDelete") return;
                        //Debug.Log($"{x.name}+{s_Instance.m_SpriteAssetReferenceLookup.ContainsValue(x)}__{s_Instance.m_SpriteAssetCount.ContainsKey(x)}");
                        if (s_Instance.m_SpriteAssetReferenceLookup.ContainsValue(x) && s_Instance.m_SpriteAssetCount.ContainsKey(x))
                        {
                            --s_Instance.m_SpriteAssetCount[x];
                            //Debug.Log($"表情资源释放{x.name}+{s_Instance.m_SpriteAssetCount[x]}");
                            if (s_Instance.m_SpriteAssetCount[x] <= 0)
                            {
                                s_Instance.m_SpriteAssetCount.Remove(x);
                                OnReleaseAsset?.Invoke(x.name);
                                List<int> hashCodes = new List<int>();
                                foreach (var (k, v) in s_Instance.m_SpriteAssetReferenceLookup)
                                {
                                    if (x == v)
                                    {
                                        hashCodes.Add(k);
                                ...
                                {
                                    if (s_Instance.m_FontMaterialReferenceLookup.ContainsKey(hashCode))
                                    {
                                        var mat = s_Instance.m_FontMaterialReferenceLookup[hashCode];
                                        // s_Instance.m_FontMaterialReferenceLookup[hashCode] = null;
                                        s_Instance.m_FontMaterialReferenceLookup.Remove(hashCode);
                                    }
                                    // s_Instance.m_SpriteAssetReferenceLookup[hashCode] = null;
                                    s_Instance.m_SpriteAssetReferenceLookup.Remove(hashCode);
                                }
                                hashCodes.Clear();
                                hashCodes = null;
                                //x = null;
