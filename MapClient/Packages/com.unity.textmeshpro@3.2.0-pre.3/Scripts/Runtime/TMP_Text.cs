...
namespace TMPro
{
    ...
    public enum SpriteAlignmentOptions
    {
        MeanLine,
        AscenderToDescender,
    }
    ...
    public abstract class TMP_Text : MaskableGraphic
    {
        ...
        #region alignImgNChractor
        public virtual float voffset
        {
            get => m_voffset;
        ...
        [SerializeField]
        protected float m_voffset;
        #endregion

        /// <summary>
        /// 记录text中的图片下标,后续用于与其它文本整体居中对齐
        /// </summary>
        protected HashSet<int> m_spriteIndices = new ();

        /// <summary>
        /// 图文混排对齐方式
        /// </summary>
        [SerializeField]
        protected SpriteAlignmentOptions m_spriteAlignmentOpts;
        public virtual bool alignSprite
        {
            get => m_enableAutoAlignSprite;
            set => m_enableAutoAlignSprite = value;
        }
        ...
        /// </summary>
        [SerializeField]
        protected bool m_enableAutoAlignSprite;
        ...
        public static event Func<TMP_Text, string, TMP_FontAsset> OnFontAssetRequest;
        public static event Func<TMP_Text, string, Material> OnMaterialRequest;
        public static event Func<TMP_Text, string, TMP_ColorGradient> OnColorGradientRequest;

        /// <summary>
        /// Event delegate to allow custom loading of TMP_SpriteAsset when using the <sprite="Sprite Asset Name"> tag.
        /// </summary>
        public static event Func<TMP_Text, string, TMP_SpriteAsset> OnSpriteAssetRequest;
        public static event Func<TMP_Text, string, TMP_SpriteAsset> OnSpriteAssetSingleRequest;
        public static event Func<TMP_Text, string, TMP_SpriteAsset> OnSpriteAssetMutlipleRequest;
        public static event Func<TMP_Text, string, TMP_SpriteAsset> OnSpriteAssetFaceRequest;
        public static event Func<TMP_Text, string, TMP_SpriteAsset> OnSpriteAssetEmojiRequest;
        public static event Action<TMP_Text, TMP_SpriteAsset> OnLoadAssetRequest;
        public static event Func<TMP_SpriteAsset> OnNullSpriteAssetRequest;
        ...
        */
        ...
        protected virtual Vector2 CalculatePreferredValues(ref float fontSize, Vector2 marginSize, bool isTextAutoSizingEnabled, TextWrappingModes textWrapMode)
        {
            ...
            for (int i = 0; i < m_TextProcessingArray.Length && m_TextProcessingArray[i].unicode != 0; i++)
            {
                ...
                if (m_isRichText && charCode == 60)  // '<'
                {
                    ...
                    if (ValidateHtmlTag(m_TextProcessingArray, i + 1, out endTagIndex, out info))
                    {
                        i = endTagIndex;
                ...
                if (m_textElementType == TMP_TextElementType.Sprite)
                {
                    ...
                    if (!string.IsNullOrEmpty(info)) m_internalCharacterInfo[m_characterCount].info = info;
        ...
        protected void AdjustLineOffset(int startIndex, int endIndex, float offset)
        {
            ...
            for (int i = startIndex; i <= endIndex; i++)
            {
                ...
                m_textInfo.characterInfo[i].meanLine -= vertexOffset.y;
        ...
        protected virtual void SaveSpriteVertexInfo(int spriteIndex)
        {
            m_textInfo.characterInfo[spriteIndex].vertex_BL.position = m_textInfo.characterInfo[spriteIndex].bottomLeft;
            m_textInfo.characterInfo[spriteIndex].vertex_TL.position = m_textInfo.characterInfo[spriteIndex].topLeft;
            m_textInfo.characterInfo[spriteIndex].vertex_TR.position = m_textInfo.characterInfo[spriteIndex].topRight;
            m_textInfo.characterInfo[spriteIndex].vertex_BR.position = m_textInfo.characterInfo[spriteIndex].bottomRight; 
        }
        ...
        /// <param name="endIndex"></param>
        /// <returns></returns>
        internal bool ValidateHtmlTag(TextProcessingElement[] chars, int startIndex, out int endIndex, out string info)
        {
            int tagCharCount = 0;
                ...
                switch ((MarkupTag)m_xmlAttribute[0].nameHashCode)
                {
                    ...
                    case MarkupTag.FONT:
                        ...
                        if (tempFont == null)
                        {
                            ...
                            tempFont = OnFontAssetRequest?.Invoke(this, new string(m_htmlTag, m_xmlAttribute[0].valueStartIndex, m_xmlAttribute[0].valueLength));
                    ...
                    case MarkupTag.SPRITE:
                        ...
                        else
                        {
                            ...
                            else
                            {
                                ...
                                {
                                    //
                                    tempSpriteAsset = OnSpriteAssetRequest?.Invoke(this,
                                        new string(m_htmlTag, m_xmlAttribute[0].valueStartIndex,
                    ...
                    case MarkupTag.SINGLE: // <single=x>
                        int spriteAssetHashCode1 = m_xmlAttribute[0].valueHashCode;
                        TMP_SpriteAsset tempSpriteAsset1 = null;
                        m_spriteIndex = 0;
                        ...
                        {
                            // A Sprite Asset has been specified
                            if (MaterialReferenceManager.TryGetSpriteAsset(spriteAssetHashCode1, out tempSpriteAsset1))
                            {
                                m_currentSpriteAsset = tempSpriteAsset1;
                            }

                            if (tempSpriteAsset1 == null)
                            {

                                tempSpriteAsset1 = OnSpriteAssetSingleRequest?.Invoke(this, new string(m_htmlTag, m_xmlAttribute[0].valueStartIndex, m_xmlAttribute[0].valueLength));
                                if (tempSpriteAsset1 == null)
                                    return false;
                                if (tempSpriteAsset1.name != "NullDontDelete")
                                    MaterialReferenceManager.AddSpriteAsset(spriteAssetHashCode1, tempSpriteAsset1);
                                m_currentSpriteAsset = tempSpriteAsset1;
                            }
                        }
                        m_spriteColor = s_colorWhite;
                    ...
                    case MarkupTag.MULTIPLE: // <mutliple=x>
                        int spriteAssetHashCode2 = m_xmlAttribute[0].valueHashCode;
                        TMP_SpriteAsset tempSpriteAsset2;
                        m_spriteIndex = -1;
                        ...
                        {
                            // A Sprite Asset has been specified
                            if (MaterialReferenceManager.TryGetSpriteAsset(spriteAssetHashCode2, out tempSpriteAsset2))
                            {
                                m_currentSpriteAsset = tempSpriteAsset2;
                            }
                            // Load Sprite Asset
                            if (tempSpriteAsset2 == null)
                            {
                                //
                                tempSpriteAsset2 = OnSpriteAssetMutlipleRequest?.Invoke(this, new string(m_htmlTag, m_xmlAttribute[0].valueStartIndex, m_xmlAttribute[0].valueLength));

                                if (tempSpriteAsset2 == null)
                                    return false;

                                //Debug.Log("Loading & assigning new Sprite Asset: " + tempSpriteAsset.name);
                                if (tempSpriteAsset2.name != "NullDontDelete")
                                    MaterialReferenceManager.AddSpriteAsset(spriteAssetHashCode2, tempSpriteAsset2);
                                m_currentSpriteAsset = tempSpriteAsset2;
                            }
                    ...
                    case MarkupTag.FACE: // <face=x>
                        int spriteAssetHashCode3 = m_xmlAttribute[0].valueHashCode;
                        TMP_SpriteAsset tempSpriteAsset3;
                        m_spriteIndex = -1;
                        ...
                        {
                            // A Sprite Asset has been specified
                            if (MaterialReferenceManager.TryGetSpriteAsset(spriteAssetHashCode3, out tempSpriteAsset3))
                            {
                                m_currentSpriteAsset = tempSpriteAsset3;
                            } // Load Sprite Asset
                            if (tempSpriteAsset3 == null)
                            {
                                //
                                tempSpriteAsset3 = OnSpriteAssetFaceRequest?.Invoke(this, new string(m_htmlTag, m_xmlAttribute[0].valueStartIndex, m_xmlAttribute[0].valueLength));

                                if (tempSpriteAsset3 == null)
                                    return false;

                                //Debug.Log("Loading & assigning new Sprite Asset: " + tempSpriteAsset.name);
                                if (tempSpriteAsset3.name != "NullDontDelete")
                                    MaterialReferenceManager.AddSpriteAsset(spriteAssetHashCode3, tempSpriteAsset3);
                                m_currentSpriteAsset = tempSpriteAsset3;
                            }
                        ...
                        for (int i = 0; i < m_xmlAttribute.Length && m_xmlAttribute[i].nameHashCode != 0; i++)
                        {
                            ...
                            switch ((MarkupTag)nameHashCode)
                            {
                                ...
                                //    break;
                                default:
                                    if (nameHashCode != 40533 && nameHashCode != 27733)
                                        return false;
                                    break;
                            }
                        }
                    ...
                    case MarkupTag.EMOJI: // <emoji=x>
                        int spriteAssetHashCode4 = m_xmlAttribute[0].valueHashCode;
                        TMP_SpriteAsset tempSpriteAsset4 = null;
                        m_spriteIndex = 0;
                        ...
                        {
                            // Load Sprite Asset
                            if (tempSpriteAsset4 == null)
                            {
                                //
                                tempSpriteAsset4 = OnSpriteAssetEmojiRequest?.Invoke(this, new string(m_htmlTag, m_xmlAttribute[0].valueStartIndex, m_xmlAttribute[0].valueLength));
                                info = new string(m_htmlTag, m_xmlAttribute[0].valueStartIndex, m_xmlAttribute[0].valueLength);
                            }

                            if (tempSpriteAsset4 == null)
                                return false;

                            if (tempSpriteAsset4.name != "NullDontDelete")
                                MaterialReferenceManager.AddSpriteAsset(spriteAssetHashCode4, tempSpriteAsset4);
                            m_currentSpriteAsset = tempSpriteAsset4;
                        }
        ...
        /// 图文水平居中对齐
        /// </summary>
        protected virtual void AlignSpriteAndCharacter()
        {
            if (!m_enableAutoAlignSprite)
                return;
            
            if (m_spriteIndices.Count == 0)
                return;
            
            TMP_TextElementType type;
            foreach (var index in m_spriteIndices)
            {
                if (m_textInfo.characterInfo[index].elementType != TMP_TextElementType.Sprite)
                    return;
                var spriteCharInfo = m_textInfo.characterInfo[index];

                int characterIdx = FindNearestCharacterInSameLine(index, spriteCharInfo.lineNumber);
                if (characterIdx == -1)
                    return;
                
                var characterInfo = m_textInfo.characterInfo[characterIdx];

                var sTL = spriteCharInfo.topLeft;
                var sBL = spriteCharInfo.bottomLeft;
                
                var sHeight = sTL.y - sBL.y;

                float alignOffset = 0;
                if (m_spriteAlignmentOpts == SpriteAlignmentOptions.MeanLine)
                {
                    alignOffset = (sHeight - characterInfo.meanLine + characterInfo.baseLine) / 2;
                    //Debug.Log($"sprite index = {index} : meanline = {characterInfo.meanLine} - baseline = {characterInfo.baseLine}");
                }
                else if (m_spriteAlignmentOpts == SpriteAlignmentOptions.AscenderToDescender)
                {
                    alignOffset = (sHeight - characterInfo.ascender + characterInfo.descender) / 2;
                    //Debug.Log($"sprite index = {index} : ascender = {characterInfo.ascender} - descender = {characterInfo.descender}");
                }
                spriteCharInfo.topLeft.y -= alignOffset;
                spriteCharInfo.bottomLeft.y -= alignOffset;
                spriteCharInfo.topRight.y = spriteCharInfo.topLeft.y;
                spriteCharInfo.bottomRight.y = spriteCharInfo.bottomLeft.y;

                m_textInfo.characterInfo[index] = spriteCharInfo;
                SaveSpriteVertexInfo(index);
            }
        }
        ...
        /// 查找图片两边最近的字符
        /// </summary>
        /// <param name="spriteIndex">图片下标</param>
        /// <param name="lineNumber">图片所在行号</param>
        /// <returns></returns>
        protected virtual int FindNearestCharacterInSameLine(int spriteIndex, int lineNumber)
        {
            int offset = 1;
            while (spriteIndex - offset >= 0 || spriteIndex + offset < m_totalCharacterCount)
            {
                TMP_CharacterInfo charInfo;
                if (spriteIndex - offset >= 0)
                {
                    charInfo = m_textInfo.characterInfo[spriteIndex - offset];
                    if( charInfo.elementType == TMP_TextElementType.Character && charInfo.lineNumber == lineNumber && charInfo.character != ' ')
                        return spriteIndex - offset;
                }

                if (spriteIndex + offset < m_totalCharacterCount)
                {
                    charInfo = m_textInfo.characterInfo[spriteIndex + offset];
                    if (charInfo.elementType == TMP_TextElementType.Character && charInfo.lineNumber == lineNumber && charInfo.character != ' ')
                        return spriteIndex + offset;
