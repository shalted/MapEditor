...
namespace TMPro
{
    ...
    {
        public static Func<string, TMP_FontAsset> GetFont;
        [SerializeField]
        ...
        private void InternalGetFontAsset()
        {
            if (m_fontAsset == null && string.IsNullOrEmpty(fontAssetKey) == false)
            {
                if (GetFont != null)
                {
                    font = GetFont(fontAssetKey);
                }
            }
        }
        ...
        protected override void LoadFontAsset()
        {
            ...
            if (!string.IsNullOrEmpty(fontAssetKey)) return;
            if (m_fontAsset == null)
            {
            ...
            */
            ...
            for (int i = 0; i < textProcessingArray.Length && textProcessingArray[i].unicode != 0; i++)
            {
                ...
                if (m_isRichText && unicode == 60) // if Char '<'
                {
                    ...
                    if (ValidateHtmlTag(textProcessingArray, i + 1, out endTagIndex, out info))
                    {
                        ...
                        if (m_textElementType == TMP_TextElementType.Sprite)
                        {
                            ...
                            if (!string.IsNullOrEmpty(info)) m_textInfo.characterInfo[m_totalCharacterCount].info = info;
        ...
        protected virtual void GenerateTextMesh()
        {
            ...
            for (int i = 0; i < m_TextProcessingArray.Length && m_TextProcessingArray[i].unicode != 0; i++)
            {
                ...
                if (m_textElementType == TMP_TextElementType.Sprite)
                {
                    ...
                    m_spriteIndices.Add(m_characterCount);
                    ...
                    if (m_currentSpriteAsset.m_FaceInfo.pointSize > 0)
                    {
                        ...
                        baselineOffset = m_currentSpriteAsset.m_FaceInfo.baseline * fontScale * m_fontScaleMultiplier * m_currentSpriteAsset.m_FaceInfo.scale + voffset;
                    ...
                    else
                    {
                        ...
                        baselineOffset = m_currentFontAsset.m_FaceInfo.baseline * fontScale * m_fontScaleMultiplier * m_currentFontAsset.m_FaceInfo.scale + voffset;
                        elementDescentLine = m_currentFontAsset.m_FaceInfo.descentLine * scaleDelta;
                    }

                    m_cached_TextElement = sprite;
                    if (!string.IsNullOrEmpty(info)) m_textInfo.characterInfo[m_characterCount].info = info;
                ...
                else if (m_textElementType == TMP_TextElementType.Character)
                {
                    ...
                    meanLineOffset = m_currentFontAsset.m_FaceInfo.meanLine * adjustedScale * m_fontScaleMultiplier * m_currentFontAsset.m_FaceInfo.scale;
                ...
                m_textInfo.characterInfo[m_characterCount].meanLine = (meanLineOffset - m_lineOffset + m_baselineOffset) + glyphAdjustments.yPlacement * currentElementScale;
            ...
            for (int i = 0; i < m_characterCount; i++)
            {
                ...
                if (isCharacterVisible)
                {
                    ...
                    int uv3x = isGrey ? 1 : 0;
                    characterInfos[i].vertex_BL.uv3.x = uv3x;
                    characterInfos[i].vertex_TL.uv3.x = uv3x;
                    characterInfos[i].vertex_TR.uv3.x = uv3x;
                    characterInfos[i].vertex_BR.uv3.x = uv3x;
                ...
                m_textInfo.characterInfo[i].meanLine += offset.y;
                // AlignSpriteAndCharacter(i);
