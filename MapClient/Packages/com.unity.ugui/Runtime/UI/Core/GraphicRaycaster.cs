...
namespace UnityEngine.UI
{
    ...
    {
        public delegate Graphic[] ProcessGraphicRaycatHandle(PointerEventData eventData, Camera eventCamera);
        private ProcessGraphicRaycatHandle m_customProcess = null;
        public ProcessGraphicRaycatHandle customProcess
        {
            get
            {
                return m_customProcess;
            }
            set
            {
                m_customProcess = value;
            }
        }
        ...
        public override void Raycast(PointerEventData eventData, List<RaycastResult> resultAppendList)
        {
            ...
            {
                Graphic[] resultList = customProcess(eventData, currentEventCamera);
                if (resultList != null)
                {
                    for (var index = 0; index < resultList.Length; index++)
                    {
                        float distance = 0;
                        var go = resultList[index].gameObject;
